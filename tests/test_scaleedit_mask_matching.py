...
from scaleedit.mask.matching import match_single_candidate, single_object_prompt
...
def test_added_attachment_requires_explicit_addition_and_same_existing_owner(source,target,description,expected):
    from scaleedit.mask.matching import added_attachment_ref
...
def test_disjoint_body_concepts_are_queried_separately_but_limb_is_not_split():
    from scaleedit.mask.matching import atomic_body_refs
...
def test_solid_object_groups_are_not_particle_tiles():
    from scaleedit.mask.pipeline import _region_mode, _mask_density, _sam_text_prompt
...
def test_pcs_single_selection_does_not_union_neighbor_pixels():
    import numpy as np
    import torch
    from scaleedit.mask.pipeline import _pcs_mask
